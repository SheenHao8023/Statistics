...
for (metric in c("IC", "WS")) {
  for (pair_name in names(group_difference_analysis[[metric]])) {
    results <- group_difference_analysis[[metric]][[pair_name]]
    
    results_summary <- rbind(results_summary, data.frame(
      Metric = metric,
      Group_Pair = pair_name,
      # 实证差异结果
      Empirical_Mean = results$empirical$posterior_mean,
      Empirical_HDI_Lower = results$empirical$hdi_lower,
      Empirical_HDI_Upper = results$empirical$hdi_upper,
      Empirical_ROPE_Lower = results$empirical$rope_lower,
      Empirical_ROPE_Upper = results$empirical$rope_upper,
      Empirical_Prob_in_ROPE = results$empirical$prob_in_rope,
      Empirical_Decision = results$empirical$decision,
      Empirical_Significant = results$empirical$significant,
      # 模拟差异结果
      Simulated_Mean = results$simulated$posterior_mean,
      Simulated_HDI_Lower = results$simulated$hdi_lower,
      Simulated_HDI_Upper = results$simulated$hdi_upper,
      Simulated_ROPE_Lower = results$simulated$rope_lower,
      Simulated_ROPE_Upper = results$simulated$rope_upper,
      Simulated_Prob_in_ROPE = results$simulated$prob_in_rope,
      Simulated_Decision = results$simulated$decision,
      Simulated_Significant = results$simulated$significant,
      # 差异的差异结果
      Diff_of_Diffs_Mean = results$difference_of_differences$posterior_mean,
      Diff_of_Diffs_HDI_Lower = results$difference_of_differences$hdi_lower,
      Diff_of_Diffs_HDI_Upper = results$difference_of_differences$hdi_upper,
      Diff_of_Diffs_ROPE_Lower = results$difference_of_differences$rope_lower,
      Diff_of_Diffs_ROPE_Upper = results$difference_of_differences$rope_upper,
      Diff_of_Diffs_Prob_in_ROPE = results$difference_of_differences$prob_in_rope,
      Diff_of_Diffs_Decision = results$difference_of_differences$decision,
      Diff_of_Diffs_Significant = results$difference_of_differences$significant,
...
# 9. 保存结果
write_csv(results_summary, file.path(file_path, "bayesian_group_difference_results.csv"))

# 10. 打印结果总结
cat("\n=== Bayesian Group Difference Analysis Results ===\n")
cat("Noninformative priors used in all models\n")
cat("ROPE set to 0 ± 0.1 * SD(y) following Kruschke (2018)\n")
cat("Statistical inferences based on 95% HDI and ROPE\n\n")

for (metric in c("IC", "WS")) {
  cat(sprintf("\n%s Results:\n", metric))
  metric_results <- results_summary %>% filter(Metric == metric)
  
  for (i in 1:nrow(metric_results)) {
    row <- metric_results[i, ]
    cat(sprintf("Group Pair: %s\n", row$Group_Pair))
    cat(sprintf("  Empirical difference: %.4f [%.4f, %.4f] | ROPE: [%.4f, %.4f]\n", 
                row$Empirical_Mean, row$Empirical_HDI_Lower, row$Empirical_HDI_Upper,
                row$Empirical_ROPE_Lower, row$Empirical_ROPE_Upper))
    cat(sprintf("    P(in ROPE) = %.3f | %s\n", row$Empirical_Prob_in_ROPE, row$Empirical_Decision))
    
    cat(sprintf("  Simulated difference: %.4f [%.4f, %.4f] | ROPE: [%.4f, %.4f]\n",
                row$Simulated_Mean, row$Simulated_HDI_Lower, row$Simulated_HDI_Upper,
                row$Simulated_ROPE_Lower, row$Simulated_ROPE_Upper))
    cat(sprintf("    P(in ROPE) = %.3f | %s\n", row$Simulated_Prob_in_ROPE, row$Simulated_Decision))
    
    cat(sprintf("  Difference of differences: %.4f [%.4f, %.4f] | ROPE: [%.4f, %.4f]\n",
                row$Diff_of_Diffs_Mean, row$Diff_of_Diffs_HDI_Lower, row$Diff_of_Diffs_HDI_Upper,
                row$Diff_of_Diffs_ROPE_Lower, row$Diff_of_Diffs_ROPE_Upper))
    cat(sprintf("    P(in ROPE) = %.3f | %s\n", row$Diff_of_Diffs_Prob_in_ROPE, row$Diff_of_Diffs_Decision))
    cat("  ---\n")
  }
}